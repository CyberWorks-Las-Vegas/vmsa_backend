...
const dirname = '/app'
const validateLogRetrievePath = path.join(dirname, 'validation', 'logs', 'logsRetrieve');
const validateLogRetrieve = require(validateLogRetrievePath);

// Load LogsRetrieve model
const LogsRetrievePath = path.join(dirname, 'models', 'LogsRetrieve');
...
router.get("/logRetrieve", async (req, res) => {
  ...
  try {
    ...
    const found = await Premises.findOne(
      { premises_id },
    );
